...
import { chmodSync, existsSync, promises as fs, writeFileSync } from "node:fs";
import path from "node:path";
import os from "node:os";
import crypto from "node:crypto";
import { z } from "zod";
// The Letta SDK spawns the `@letta-ai/letta-code` CLI as a subprocess via
// `spawn("node", <path>)`, so the file must exist on disk at runtime. On
// Vercel the package isn't installed into the function's node_modules
// (eve's externalDependencies hook stops bundling, but Vercel's
// experimentalServices doesn't ship the package), so we materialize the CLI
// from an inlined copy of its source (generated by scripts/inline-letta-cli.mjs)
// into /tmp on cold start.
import { LETTA_CLI_SOURCE_BASE64 } from "../.generated/letta-cli-content";
...
function locateCli(): string | null {
  ...
  // Last-resort fallback: write the inlined CLI source to /tmp and use that.
  // This handles the Vercel case where the package was never installed on
  // the function's runtime filesystem.
  try {
    const tmpPath = path.join(os.tmpdir(), "letta-cli.js");
    if (!existsSync(tmpPath)) {
      const cliSource = Buffer.from(LETTA_CLI_SOURCE_BASE64, "base64").toString(
        "utf8",
      );
      writeFileSync(tmpPath, cliSource, "utf8");
      chmodSync(tmpPath, 0o755);
      console.log(
        `[letta-memory] materialized bundled CLI at ${tmpPath} (${cliSource.length} bytes)`,
      );
    }
    return tmpPath;
  } catch (err) {
    console.warn(
      `[letta-memory] failed to materialize bundled CLI: ${(err as Error).message}`,
    );
  }
